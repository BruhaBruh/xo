...
const createRoom = async (
  ...
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_SERVER_URL}/api/v1/matrix`,
    {
      cache: 'no-cache',
      method: 'POST',
      headers: {
...
export const GET = async (req: NextRequest): Promise<NextResponse> => {
  ...
  if (!room) return NextResponse.redirect(new URL('/matrix', req.url));

  const res = NextResponse.redirect(
    new URL(`/matrix/online/${room.info.code}`, req.url)
  );
